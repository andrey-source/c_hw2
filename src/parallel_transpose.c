...
bool fill_args(args *struct_args, double *matrix, double *copy_matrix, size_t left,
            size_t right, size_t n_rows, size_t n_columns) {
...
bool transpose(double *matrix, size_t *n_rows, size_t *n_columns) {
    ...
    for (size_t i = 0; i < num_pth - 1; i++) {
        bool fill_flag = fill_args(&args_butch[i], matrix, copy_matrix,
            size_butch * i, size_butch * (i + 1), *n_rows, *n_columns);
        if (!fill_flag) {
            free(pth);
            free(copy_matrix);
            free(args_butch);
